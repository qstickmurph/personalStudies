...
.text
pr:
      jal input 	    # run input subroutine
      la   $s0, fibs        # load address of array
      la   $s6, size
      lw   $s5, 0($s6)        # load address of size variable
...
###############################################################
# Subroutine to prompt the user for input
input: 
      la $a0, prompt
      li $v0, 4
      syscall
...
.data
	...
	head1: .asciiz  "The Fibonacci numbers are:\n"
.text

print:add  $t0, $zero, $a0  # starting address of array of data to be printed
      add  $t1, $zero, $a1  # initialize loop counter to array size
      la   $a0, head1       # load address of the print heading string
